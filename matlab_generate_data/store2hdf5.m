function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, LL, LH, LV, DD, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/CA', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/CH', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/CV', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/CD', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
      startloc.dat=[ones(1,length(dat_dims)-1), 1];
      startloc.CA=[ones(1,length(lab_dims)-1), 1];
      startloc.CH=[ones(1,length(lab_dims)-1), 1];
      startloc.CV=[ones(1,length(lab_dims)-1), 1];
      startloc.CD=[ones(1,length(lab_dims)-1), 1];
    end 
  else  % append mode
    if ~exist('startloc','var')
      ...
      startloc.CA=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.CH=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.CV=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.CD=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
    end
  end

  if ~isempty(data)
    h5write(filename, '/data', single(data), startloc.dat, size(data));
    h5write(filename, '/CA', single(LL), startloc.CA, size(LL));  
    h5write(filename, '/CH', single(LH), startloc.CH, size(LH));  
    h5write(filename, '/CV', single(LV), startloc.CV, size(LV));  
    h5write(filename, '/CD', single(DD), startloc.CD, size(DD));  
  end
